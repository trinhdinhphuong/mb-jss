/*
 * ContentHover jQuery plugin
 * Phát triển từ http://www.backslash.gr/demos/contenthover-jquery-plugin/
 * Minh Bang
 */
(function ($) {
    var methods = {
        init: function (options) {
            var defaults = {
                    full_width: true, // contenthover mở rộng full width thay vì bằng hình ảnh
                    full_height: false, // contenthover mở rộng full height thay vì chỉ show phần content
                    title: false, // false | title selector: contenthover không ẩn hết, mà chừa lại phần title
            ...
            return this.each(function () {
                var $this = $(this),
                    _height = Math.min($this.parent().height(), $this.height()),
                    w = $this.width() ? $this.width() : settings.width,
                    h = _height ? _height : settings.height,
                    ...
                    title_h = settings.title ? $(settings.title, $data).innerHeight() : 0,
                    hover_top = settings.full_height ? 0 : Math.max(0, h - $data.innerHeight());

                if ($data.length) {
                    $data.hide();

                    var $ch_wrapper = $('<div>').addClass('ch_element').addClass(settings.wrapper_class).css({
                        'width': settings.full_width ? '100%' : w,
                        'height': h,
                        'position': 'relative',
                        'overflow': 'hidden'
                    }).insertAfter($this);
                    ...
                    var $ch_hover = $('<div>').addClass(settings.hover_class).css({
                        'width': settings.full_width ? '100%' : overlay_w,
                        'height': overlay_h,
                        'position': 'absolute',
                    ...
                    if (settings.overlay_y_position == 'top') {
                        ...
                        ch_hover_css.top = (h - overlay_h + hover_top) + 'px';
                    } else {
